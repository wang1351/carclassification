...
def train_epoch(model, current_epoch, criterion, optimizer, dataset, args):
    ...
    for i, data in enumerate(trainloader):
        ...
        running_loss = 0
    with open('train_epoch_'+str(current_epoch)+'_acc.txt', 'w') as f:
        for item in train_acc:
            f.write("%s\n" % item)

    with open('train_epoch_'+str(current_epoch)+'_loss.txt', 'w') as f:
            ...
            if (i % args['log_freq']) == 0:
                ...
                print('Epoch: {:03d} \t Iteration: {:03d}/{:03d} \t Loss: {:.05f}  \t Acc: {:.05f}'.format(current_epoch, i + 1,
                                                                                                           ...
                                                                                                           running_loss / (
                                                                                                                       i + 1),
                                                                                                           acc))
        with open('val_epoch_'+str(current_epoch)+'_acc.txt', 'w') as f:
            for item in val_acc:
                f.write("%s\n" % item)
...
if __name__ == '__main__':
    ...
    classification_train = CarType_Classification(split_file='./train.txt', transforms=train_transform)

    classification_val = CarType_Classification(split_file='./test.txt', transforms=val_transform)
    ...
    sharedModel = ResNet18(n_classes=163)
    # model = AlexNet(num_classes=163)
    # model = models.alexnet(num_classes=163)
    PATH_TO_MODEL = os.path.join(args['check_point_out_dir'], 'CompCar_Res18', 'best_so_far_res.pth')
    weights = torch.load(PATH_TO_MODEL)['model_state_dict']
    sharedModel.load_state_dict(weights)
    sharedModel = sharedModel.to(device)

    model = multiRes(sharedModel=sharedModel, num_classes=9)
