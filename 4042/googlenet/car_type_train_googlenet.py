...
def train_epoch(model, current_epoch, criterion, optimizer, dataset, args):
    ...
    for i, data in enumerate(trainloader):
        ...
        running_loss = 0
    with open('4train_epoch_'+str(current_epoch)+'_acc.txt', 'w') as f:
        for item in train_acc:
            f.write("%s\n" % item)

    with open('4train_epoch_'+str(current_epoch)+'_loss.txt', 'w') as f:
            ...
            if (i % args['log_freq']) == 0:
                ...
                print('Epoch: {:03d} \t Iteration: {:03d}/{:03d} \t Loss: {:.05f}  \t Acc: {:.05f}'.format(current_epoch, i + 1,
                                                                                                           ...
                                                                                                           running_loss / (
                                                                                                                       i + 1),
                                                                                                           acc))
        with open('4val_epoch_'+str(current_epoch)+'_acc.txt', 'w') as f:
            for item in val_acc:
                f.write("%s\n" % item)
...
if __name__ == '__main__':
    ...
    model = GoogLeNet(num_classes=9)
    PATH_TO_MODEL = os.path.join(args['check_point_out_dir'], 'googlenet_type', 'best_so_far.pth')
    ...
    for epoch in range(args['num_epoch']):
        ...
        if val_acc > current_best_val_acc:
            ...
            torch.save({'epoch': epoch,
                        ...
                        'val_loss': val_loss,
                        'val_acc': val_acc
                        }, os.path.join(args['check_point_out_dir'], 'googlenet_type', name))
